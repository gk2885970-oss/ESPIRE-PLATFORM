...
from sqlalchemy import String, Integer,ForeignKey,Text,Table,Column,DateTime,func,Date,TIMESTAMP,Float,JSON
...
class MatchPerformance(Base):
    ...
    result:Mapped[str] = mapped_column(String(27), nullable=False)
...
class Team(Base):
    ...
    team_name:Mapped[str]=mapped_column(String(20),unique=True)
    email:Mapped[str]=mapped_column(String(100),unique=True)
    hash_password:Mapped[str]=mapped_column(Text,nullable=False)
    # match:Mapped[List["Match"]]=relationship(back_populates="teams")
...
class Map(Base):
    __tablename__="maps"
    id:Mapped[int]=mapped_column(Integer,primary_key=True,autoincrement=True)
    name:Mapped[Map_FreeFireMax]= mapped_column(Enum(Map_FreeFireMax,name="freefire_map",native_enum=False),unique=True,nullable=False)
    # performances:Mapped[List["MatchPerformance"]] = relationship("MatchPerformance", back_populates="map")

    # matches:Mapped[List["Match"]] = relationship(
    # secondary="matchPerformance", back_populates="maps")
    # def __repr__(self):
    #     return f"<map_id: {self.id}, map_name: {self.name}>"
